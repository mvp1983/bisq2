...
abstract class JPackageTask : DefaultTask() {
    ...
    fun run() {
        ...
        // Generate AUR package when on Linux
        if (getOS() == OS.LINUX) {
            ...
            if (packageFormatConfigs is LinuxPackages &&
                ...
                logger.lifecycle("Generating AUR package for ${appName.get()} ${appVersion.get()}")
                ...
                logger.lifecycle("AUR package generated at ${outputDirectory.asFile.get().toPath().resolve("aur")}")
            }
        }
    }
